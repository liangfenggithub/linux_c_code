                                     ...
                                     void *eloop_data, void *user_data)
{
    ...
    tmp = os_realloc_array(table->table, (size_t)(table->count + 1), sizeof(struct eloop_sock));
    if (tmp == NULL) {
                     ...
                     fd_set *fds)
{
    ...
    for (i = 0; i < table->count; i++) {
        ...
        FD_SET((long unsigned int)(table->table[i].sock), fds);
    }
}
...
void eloop_wait_for_read_sock(int sock)
{
    ...
    FD_ZERO(&rfds);
    FD_SET((long unsigned int)sock, &rfds);
