 ...
 *
 * 取消匹配<handler,eloop_data,user_data>的第一个timeout，一般是最近要到期的那个
 * remaining返回剩余时间
 */
 ...
 * 
 * 找到匹配<handler,eloop_data,user_data>的timeout处理，
 * 如果新时间比旧时间早，则取消旧处理，使用新时间重新注册
 */
